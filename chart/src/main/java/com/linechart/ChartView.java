...
public class ChartView extends View {
    public static final String TAG="nihaoA";
    //xy坐标轴颜色
...
//            if (i == selectIndex - 1) {
    ...
    private void drawFloatTextBox(Canvas canvas, float x, float y, float text) {
        ...
        Rect rect = getTextBounds(showText + "", linePaint);
        canvas.drawText(showText + "", x - rect.width() / 2, y - dp6 - (dp18 - rect.height()) / 2, linePaint);
    }
...
//        canvas.drawPath(path, xyPaint);
        ...
        for (int i = 0; i < yValue.size(); i++) {
            //绘制Y轴刻度
            canvas.drawLine(xOri, yOri - yLength * i + xylinewidth / 2, xOri + xLength, yOri - yLength * i + xylinewidth / 2, xyPaint);
            xyTextPaint.setColor(xytextcolor);
            //绘制Y轴文本
...
//        canvas.drawPath(path, xyPaint);
        ...
        for (int i = 0; i < xValue.size(); i++) {
            ...
            if (x >= xOri) {//只绘制从原点开始的区域
                xyTextPaint.setColor(xytextcolor);
                canvas.drawLine(x, yOri, x, yOri - yLength * (yValue.size()-1) + xylinewidth / 2, xyPaint);
                //绘制X轴文本
     ...
     * 回收加速度测试器，防止内存泄漏
     */
    private void recycleVelocityTracker0() {
        if (mVelocityTracker != null) {
...
//            return super.onTouchEvent(event);
        ...
        switch (event.getAction()) {
            ...
            case MotionEvent.ACTION_MOVE:
                Log.d(TAG, "ChartView onTouchEvent: xValue.size="+xValue.size());
...
//                Log.d(TAG, "onTouchEvent: 加速度="+pxsec);
//
//                //获取滑动的最大滑动距离（最后一个Child的右边框的坐标减去屏幕的宽度）
//                int finalyChild = (int) (interval*xValue.size()+xOri- screenWidth);
//                //如果x的加速度大于系统设定的最小移动距离，就可以惯性滑动
//                if (Math.abs(pxsec) > 1000)
//                {
//                    mScroller.fling(getScrollX(), 0, (int) pxsec, 0, 0, finalyChild, 0, 0);
//                    Log.d(TAG, "onTouchEvent: 大于 最小速度");
//                }
//
...
//        Log.d(TAG, "onTouchEvent: s加速度="+velocity);
        float scrollLength = maxXInit - minXInit;
//        Log.d(TAG, "ChartView scrollAfterActionUp: origin srollLength="+scrollLength);
        float vis=velocity;
        if (Math.abs(vis)>10000)
            vis=vis>0?10000:-10000;
        if (Math.abs(vis)<1500)
            return;
...
//                Log.d(TAG, "onAnimationUpdate: Value="+value);
                if (velocity < 0 && xInit > minXInit) {//向左滑动
                    if (xInit - value <= minXInit)
                        ...
                        xInit = xInitTemp - value;
//                    Log.d(TAG, "onAnimationUpdate: XInit="+xInit);
...
//                    Log.d(TAG, "onAnimationUpdate: XInit="+xInit);
    ...
    private float getVelocity() {
        ...
        if (velocityTracker != null) {
            ...
            v= velocityTracker.getXVelocity();
    ...
    public void setValue(Map<String, Float> value, List<String> xValue, List<Integer> yValue) {
        this.value = value;
        this.xValue = xValue;
        this.yValue = yValue;
       //重要
