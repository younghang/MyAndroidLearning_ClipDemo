 ...
 */
public abstract class BaseCalendarListAdapter<T> extends com.example.yanghang.clipboard.OthersView.calendarlistview.library.SectionedBaseAdapter {
    ...
    protected com.example.yanghang.clipboard.OthersView.calendarlistview.library.CalendarListView.OnCalendarViewItemClickListener.SelectedDateRegion getSelectedDateTypeByDate(String date) {
        String key = indexToTimeList.get(0);
        if (key.compareTo(date) > 0) {
            return com.example.yanghang.clipboard.OthersView.calendarlistview.library.CalendarListView.OnCalendarViewItemClickListener.SelectedDateRegion.BEFORE;
        }
        key = indexToTimeList.get(indexToTimeList.size() - 1);
        if (key.compareTo(date) < 0) {
            return com.example.yanghang.clipboard.OthersView.calendarlistview.library.CalendarListView.OnCalendarViewItemClickListener.SelectedDateRegion.AFTER;
        }
    ...
    public View getSectionHeaderView(String date, View convertView, ViewGroup parent) {
        ...
        if (convertView != null) {
            ...
            convertView = inflater.inflate(R.layout.listitem_calendar_header1, null);
